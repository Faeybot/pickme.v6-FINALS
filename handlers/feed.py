...
# Pastikan filter ini ada di utils Anda, atau hapus baris ini jika tidak dipakai
...
# ==========================================
# 2. FORMATTER POSTINGAN (DENGAN FIX DEEP LINK)
# ==========================================
...
async def render_feed_ui(bot: Bot, chat_id: int, user_id: int, db: DatabaseService, state: FSMContext, callback_id: str = None):
    ...
    text = (
        ...
        f"📦 <b>Sisa Kuota Extra (Talent/Premium):</b>\n"
...
# ==========================================
# 4. HANDLER MENU FEED & PEMBATALAN
# ==========================================
...
async def handle_text_input(message: types.Message, state: FSMContext, db: DatabaseService, bot: Bot):
    try: await message.delete() # Hapus chat user agar bersih
    except: pass
...
async def process_publish(callback: types.CallbackQuery, state: FSMContext, db: DatabaseService, bot: Bot):
    ...
    async with db.session_factory() as session:
        ...
        if post_type == "text":
            ...
            await callback.message.edit_caption(caption="✅ <b>Postingan teks berhasil diterbitkan!</b>", reply_markup=kb_done, parse_mode="HTML")
            await state.clear()
        else:
