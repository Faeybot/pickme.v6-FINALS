...
# --- 1B. APPROVAL TRIAL (STRATEGI JACKPOT VIP+ 7 HARI) ---
...
async def admin_view_profile(callback: types.CallbackQuery, db: DatabaseService):
    ...
    status = "💎 VIP+" if user.is_vip_plus else "🌟 VIP" if user.is_vip else "🎭 PREMIUM" if user.is_premium else "👤 FREE"
    
    info = (
        f"👤 <b>ADMIN VIEW: {user.full_name}</b>\n"
        f"Kasta: {status}\n"
        f"Saldo: {user.points:,} Poin\n"  # Disesuaikan dengan update database.py terbaru
...
async def ask_broadcast_message(callback: types.CallbackQuery, state: FSMContext):
    ...
    try: await callback.message.edit_text(text, parse_mode="HTML")
    except: pass
...
async def execute_force_reset(callback: types.CallbackQuery, db: DatabaseService, bot: Bot):
    ...
    for user_id in all_users:
        try:
            ...
            await render_dashboard_ui(bot, user_id, user_id, db, is_new_message=True)
            success_count += 1
        except Exception:
            pass 
        await asyncio.sleep(0.05)
