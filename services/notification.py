...
class NotificationService:
    ...
    async def send_push_alert(self, target_id: int, alert_type: str):
        """Kirim notifikasi push sederhana (tanpa inline button)"""
        if alert_type == "LIKE":
    ...
    async def trigger_unmask(self, target_id: int, sender_id: int):
        ...
        try:
            await self.bot.send_message(target_id, text, reply_markup=kb, parse_mode="HTML")
        except:
            pass
    ...
    async def trigger_new_message(self, target_id: int, sender_id: int, sender_name: str, is_reply: bool = False):
        ...
        if user and user.nav_stack:
            if user.nav_stack and user.nav_stack[-1] == f"chat_room_{sender_id}":
                return
            if user.nav_stack and user.nav_stack[-1] == "inbox":
                return
